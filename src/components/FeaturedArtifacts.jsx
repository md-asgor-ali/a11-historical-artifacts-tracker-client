...
// ---------------- Featured Card ----------------
const FeaturedCard = ({ artifact, index, isTopRated, hoveredCard, setHoveredCard }) => {
  const cardVariants = {
    ...
    >
      ...
      <div className="relative h-56 overflow-hidden">
        ...
        <div className="absolute top-4 right-4">
          <motion.div className="flex items-center space-x-1 px-2 py-1 rounded-full bg-black/50 backdrop-blur-sm border border-white/20 text-white" whileHover={{ scale: 1.1 }}>
        ...
        {(artifact.likeCount || 0) > 0 && (
          <div className="absolute bottom-4 left-4">
            <motion.div className="flex items-center space-x-1 px-2 py-1 rounded-full bg-gradient-to-r from-green-500/80 to-green-600/80 text-white text-xs font-bold"
              initial={{ scale: 0 }}
              animate={{ scale: 1 }}
      ...
      <div className="p-6 relative z-10">
        <motion.h3 className="text-xl font-bold text-white mb-3 line-clamp-2 group-hover:text-amber-300 transition-colors duration-300">
          {artifact.name}
        </motion.h3>
        <p className="text-gray-400 text-sm mb-4 line-clamp-3 leading-relaxed">
          {artifact.shortDesc || artifact.description}
        </p>
      ...
      {/* Hover glow */}
      <div className={`absolute inset-0 rounded-2xl transition-opacity duration-500 pointer-events-none ${isTopRated
...
const FeaturedArtifacts = () => {
  ...
  useEffect(() => {
    axios
      .get("https://a11-historical-artifacts-tracker-se.vercel.app/artifacts/featured")
      .then((res) => {
  ...
  const topArtifacts = useMemo(() => {
    ...
    return [...featuredData].sort((a, b) => (b.likeCount || 0) - (a.likeCount || 0)).slice(0, 6);
  }, [featuredData]);

  if (loading) return <Loading />;

  if (!topArtifacts.length) return (
    <div className="text-center py-16">
      <Sparkles className="h-16 w-16 text-gray-400 mx-auto mb-4" />
      <h3 className="text-2xl font-bold text-white mb-2">No Featured Artifacts</h3>
  ...
  return (
    <section className="py-20 px-4 sm:px-6 lg:px-8 relative overflow-hidden"
      style={{ background: "linear-gradient(135deg, rgba(0,0,0,0.95) 0%, rgba(20,20,20,0.98) 50%, rgba(0,0,0,0.95) 100%)" }}
    >
      ...
      {[Award, Crown, Flame, TrendingUp, Sparkles].map((Icon, i) => (
        <motion.div
          ...
          transition={{ duration: 6 + i * 0.5, repeat: Number.POSITIVE_INFINITY, delay: i * 1.2 }}
        >
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        <motion.div className="text-center mb-16" initial={{ opacity: 0, y: -50 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
          ...
          <h2 className="text-4xl md:text-5xl font-bold text-white mb-6 tracking-wide">
            Featured <span className="bg-gradient-to-r from-amber-400 via-amber-300 to-amber-500 bg-clip-text text-transparent">Artifacts</span>
          </h2>

          <p className="text-xl text-gray-300 max-w-3xl mx-auto leading-relaxed">
            Discover the most appreciated artifacts in our collection, loved by researchers and history enthusiasts worldwide
          </p>

          {/* Stats */}
          <div className="flex justify-center items-center space-x-8 mt-8">
            <div className="text-center">
              <div className="text-2xl font-bold text-amber-400">{topArtifacts.length}</div>
              <div className="text-sm text-gray-400">Featured Items</div>
            </div>
            <div className="text-center">
              <div className="text-2xl font-bold text-amber-400">
                {topArtifacts.reduce((sum, a) => sum + (a.likeCount || 0), 0)}
              </div>
              <div className="text-sm text-gray-400">Total Likes</div>
            </div>
            <div className="text-center">
              <div className="text-2xl font-bold text-amber-400">{new Set(topArtifacts.map(a => a.type)).size}</div>
        ...
        <motion.div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mb-16" variants={{ hidden: { opacity: 0 }, visible: { opacity: 1, transition: { staggerChildren: 0.1, delayChildren: 0.2 } } }} initial="hidden" animate="visible">
        ...
        {/* See All Button */}
        <motion.div className="text-center" initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.8, duration: 0.6 }}>
          <Link to="/all-artifacts" className="group inline-flex items-center space-x-3 px-8 py-4 rounded-xl bg-gradient-to-r from-amber-500 to-amber-600 text-black font-bold text-lg hover:from-amber-400 hover:to-amber-500 transition-all duration-300 shadow-lg shadow-amber-600/30 hover:shadow-amber-500/40">
